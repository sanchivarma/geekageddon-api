...
export function getBorderSVG(type = 'none', theme = {}, width = 480, height = 160, padding = 24, reduceMotion = false, borderImage = null) {
  ...
  // Alternating rainbow pulsating dots (8 around the card)
  if (type === 'rainbow-pulse-dots' || type === 'alt-rainbow-dots') {
    ...
    const inset = 6;
    const positions = [
      [innerX + inset, innerY + inset],
      [innerX + innerW/2, innerY + inset],
      [innerX + innerW - inset, innerY + inset],
      [innerX + innerW - inset, innerY + innerH/2],
      [innerX + innerW - inset, innerY + innerH - inset],
      [innerX + innerW/2, innerY + innerH - inset],
      [innerX + inset, innerY + innerH - inset],
      [innerX + inset, innerY + innerH/2]
    ];
    const base = `<rect x="${innerX}" y="${innerY}" width="${innerW}" height="${innerH}" rx="${rx}" fill="none" stroke="${shadeColor(borderColor, -40)}" stroke-width="1.5" />`;
    const dots = positions
      .map(([cx, cy], idx) => {
        const delay = reduceMotion ? '' : `animation-delay: ${idx * 150}ms;`;
