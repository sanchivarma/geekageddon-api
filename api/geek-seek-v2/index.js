...
export default async function handler(req, res) {
  ...
  const radiusMeters = parseNumber(req.query.radiusMeters ?? req.query.radius, DEFAULT_RADIUS_METERS);
  const languageCode = req.query.languageCode ?? DEFAULT_LANGUAGE;
  const regionCode = req.query.regionCode ?? DEFAULT_REGION;
  const includedTypes = parseArray(req.query.type ?? DEFAULT_TYPE).slice(0, 5);
  ...
  try {
    ...
    const query = req.query.query ?? req.query.q ?? "Pharmacy in Berlin";
